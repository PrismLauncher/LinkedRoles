...
app.get('/discord-oauth-callback', async (req, res) => {
  try {
    ...
    const { url, state } = github.getOAuthUrl();
 ...
 * 3. Store the Github tokens in redis / Firestore
...
app.get('/github-oauth-callback', async (req, res) => {
  try {
    ...
    const { discordUserId } = await storage.getStateData(state);

    // 2. Use the code in the querystring to acquire Github OAuth2 tokens
    const code = req.query['code'] as string;
    const tokens = await github.getOAuthTokens(code);
 ...
 * https://dev.github.com/build/reference/web-api/developer-guide/using-subscriptions/#Verifying-a-Subscriber
 */
 ...
 * Takes a few steps:
 * 1. Fetch the Discord and Github tokens from storage (redis / firestore)
